...
import { IStoryAction, IIterationAction, IIteration, IEpicAction, IStoryInEpic, IStoryMapAction } from "../../StoryMap/interfaces";
import { IEpicInfo, IIterationInfo } from "../../StoryMap/interfaces";
...
const iterationReducer: Reducer<{
  ...
  switch (action.type) {
    ...
    case 'storyMap-modifyIteration':
      state.iterations = state.iterations.map((iteration: IIteration) => action.iteration.id === iteration.id ? {...iteration, ...action.iteration} : iteration);
      break;
  }
  return state;
}
